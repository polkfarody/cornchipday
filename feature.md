...
- **Real fairness bug found via playtest and fixed:** Grease Splatter originally patrolled like a normal enemy, but since it's unstompable, the danger zone a player had to clear was the patrol sweep plus its hit-box on both ends -- far wider than a single jump could reliably clear, and no Air Fryer exists past Level 1 to defeat it with instead (spin-dash doesn't persist across the scene reload between levels). Fixed by making it stationary by default (`move_speed = 0` in `GreaseSplatterEnemy.tscn`) -- a fixed hazard is fair to learn and jump over; a moving unstompable one isn't.
