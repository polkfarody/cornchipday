...
- [x] Wrote up a proposed 7-level bestiary (`characters.txt` Bestiary by Level) so the enemy roster evolves per level instead of repeating — not yet built beyond Level 1, but the ingredient order and per-level enemy/boss list are now concrete
...
- [ ] Levels 2–6, each per `characters.txt` Bestiary by Level: Cheese Kitchen, Guac Stand, Market Tomatoes, Frosty Fridge, Sizzling Griddle — each needs its own new boss (Queso Grande, Big Red, Sour Cream Sam, Iron Skillet; Avocado already has art) and new enemies (Jalapeño, Lime, Onion, Cherry Tomato, Ice Cube, Chive Bit, Grease Splatter all still need art + `boss.gd`-style instances)
...
- [ ] Cheese's proper "sleep for 10 seconds" effect, planned for its Level 2 reappearance (see `feature.md` FB14)
...
- The Bestiary by Level plan (`characters.txt`) is proposed, not confirmed — review before building Levels 2–6 on top of it.
