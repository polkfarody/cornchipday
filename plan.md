...
- [x] Wrote up a proposed 7-level bestiary (`characters.txt` Bestiary by Level) so the enemy roster evolves per level instead of repeating — **this whole bestiary, including every named character in it beyond what's already built, was AI-invented and is not yet confirmed by the user.** Treat every name in it as a placeholder proposal, not a decision, until confirmed.
...
- [ ] Levels 2–6 — **entirely unconfirmed roster**, see Open Questions. If confirmed as proposed: Cheese Kitchen, Guac Stand, Market Tomatoes, Frosty Fridge, Sizzling Griddle, each with its own boss (Queso Grande, Big Red, Sour Cream Sam, Iron Skillet; Avocado already has art) and new enemies (Jalapeño, Lime, Onion, Cherry Tomato, Ice Cube, Chive Bit, Grease Splatter — none have art or code yet except Queso Grande/Jalapeño, which are themselves unconfirmed)
- [ ] Level 7: no separate ingredient boss, built entirely around the Wrap confrontation, preceded by a remix gauntlet of one earlier enemy per level — also unconfirmed
...
- **The entire Bestiary by Level plan (`characters.txt`) is an AI-invented proposal, not a confirmed design.** Nothing in Levels 2–7 should be built until the user has reviewed and confirmed it, per instructions-ai.txt Operating Principle 4.
- **Queso Grande and Jalapeño already have generated art**, produced before this confirmation was sought. The user needs to decide: keep these two specifically (art already exists, no additional cost to use them), drop them in favor of different concepts, or workshop them together before proceeding.
