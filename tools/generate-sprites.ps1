...
$sprites = @(
    ...
    @{ Name = "l3_window_view"; Prompt = "A rustic wooden-plank wall (like a farm produce stand), filling the entire canvas edge-to-edge with no gaps or empty background, with a plain window opening cut into it showing a hazy, softer-detailed distant scene beyond: silhouettes of more produce stands and crates under daylight. No characters, faces, or creatures anywhere in the scene -- empty scenery only. $styleSuffix"; NoRefs = $true; Model = "gemini-3-pro-image" },
    ...
    @{ Name = "l4_window_view"; Prompt = "A red-and-white striped cantina awning mounted on a warm terracotta wall, filling the entire canvas edge-to-edge with no gaps or empty background, with a plain window opening cut into the awning showing a hazy, softer-detailed distant scene beyond: silhouettes of more market stalls and rooftops under warm daylight. No characters, faces, or creatures anywhere in the scene -- empty scenery only. $styleSuffix"; NoRefs = $true; Model = "gemini-3-pro-image" },
    ...
    @{ Name = "l5_window_view"; Prompt = "A frosted metal fridge-panel wall, filling the entire canvas edge-to-edge with no gaps or empty background, with a plain shelving-frame opening showing a hazy, softer-detailed distant scene beyond: silhouettes of more shelving deeper in a walk-in fridge under cool blue light with soft fog. No characters, faces, or creatures anywhere in the scene -- empty scenery only. $styleSuffix"; NoRefs = $true; Model = "gemini-3-pro-image" },
